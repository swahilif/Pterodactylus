...
void* interpret(MethodEntry* method, pClass* cl, StackFrame* frame, ulong* thread_stack){
    ...
    while(count < code_length){
        ...
        switch(bytecode){
            ...
            case (0xac):{
                int *res = new int;
                *res = thread_stack[frame->stack_top];
                return (void*)res;
            }
            // return
            case (0xb1):{
            ...
            case(0xb4):{
                ...
                string type = nat.substr(type_pos+1, nat.length() - type_pos + 1);
                Object *obj = reinterpret_cast<Object*>(thread_stack[frame->stack_top]);
                ulong res = (long)obj->getField(nat);
            ...
            case(0xb6):{
                ...
                if (!new_cl){
                    /*
                    class_attribute* new_class = new class_attribute((char*)class_name.c_str());
                    if (!ClassLoader::LoadClass(new_class))
                        throw "Load Class Failed!";
                    new_cl = ClassLoader::findLoadedClass(class_name);
                    */
